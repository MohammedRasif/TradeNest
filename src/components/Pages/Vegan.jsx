...
const Vegan = () => {
  ...
  return (
    <div className="lg:mx-36 md:mx-10 mx-5 my-5">
        <div className="mb-5 flex items-center justify-between">
        <h5 className="text-xl font-bold ">Vegan Meat</h5>
        <Link to="/veganMeat"><button className="text-sm text-slate-500 ">All Products</button></Link>
        </div>
        <Swiper
  slidesPerView={1} // Default for small screens
  spaceBetween={10} // Less space for small devices
  breakpoints={{
  ...
  {footData.slice(0, 10).map((foots) => (
    <SwiperSlide key={foots.id}>
      <Link to={`/veganMeat/${foots.id}`}>
        <div className="card bg-base-100 lg:w-64 md:w-56 w-full h-auto shadow-lg border rounded-lg relative group overflow-hidden transition-all duration-300">
